    ...
    if len(sys.argv) < 3:
        ...
        while not ctrl_c:
            ...
            if x_data1 == 0:
                ...
                setpoint_stop = 1.5
                kp = 1.8;ki = 0.006;kd = 0.01;    upper = 0.46; lower = 0.0
                cmd.linear.x = pid(trans[0],setpoint_stop,kp,ki,kd,dt,upper,lower)
                setangle_stop = 0
                cmd.angular.z = pid(trans[1],setangle_stop,kp,ki,kd,dt,upper,lower)
                last_time = current_time
                rospy.loginfo("Slow and Stop!!")
                rospy.loginfo("Current vel %s", cmd.linear.x)
                rospy.loginfo("dt %s", dt)
            else:
                if x_data1 == 1 and y_data == 0:
                    current_time = time.time()
                    dt = current_time - last_time
                    setpoint_stop = 1.5
                    kp = 1.8;ki = 0.001;kd = 0.01;upper = 0.46; lower = 0.0
                    cmd.linear.x = pid(trans[0],setpoint_stop,kp,ki,kd,dt,upper,lower)
                    setangle_stop = 0
                    cmd.angular.z = pid(trans[1],setangle_stop,kp,ki,kd,dt,upper,lower)
                    last_time = current_time
